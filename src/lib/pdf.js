...
export async function downloadPDF(doc, profile, client) {
  ...
  // Fetch signature image if doc is signed and has signature_url
  if (doc.status === "signed" && doc.signature_url) {
    try {
      const response = await fetch(doc.signature_url);
      if (response.ok) {
      ...
      // Failed to fetch signature — generate PDF without it
    }
  }
...
export function getPDFBlob(doc, profile, client) {
  const pdf = generatePDF({ document: doc, profile, client });
